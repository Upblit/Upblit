...
function getCliAuthUrl(provider: "github" | "google", redirectUri: string) {
  const backendBase = getApiBaseUrl();
  const path = `/auth/cli/start?provider=${encodeURIComponent(provider)}&redirectUri=${encodeURIComponent(redirectUri)}`;
  if (!backendBase || !/^https?:\/\//i.test(backendBase)) {
    return path;
  }
...
export function LoginForm({
  ...
  cliRedirectUri,
  ...props
}: React.ComponentProps<"form"> & { initialError?: string | null; cliRedirectUri?: string | null }) {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      window.location.assign(
        cliRedirectUri ? getCliAuthUrl("google", cliRedirectUri) : getGoogleAuthUrl()
      );
    } catch (oauthError) {
      setError(
  ...
  function handleGithubLogin() {
    ...
    window.location.assign(
      cliRedirectUri ? getCliAuthUrl("github", cliRedirectUri) : getGithubAuthUrl()
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5">
        <label
          htmlFor="login-email"
          className="block text-[0.7rem] font-semibold uppercase tracking-widest text-white/50"
        >
          Email address
        </label>
        <input
