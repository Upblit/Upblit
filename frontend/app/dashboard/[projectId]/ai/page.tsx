    ...
    <SidebarProvider>
      ...
      <SidebarInset className="bg-[#0a0a0a]">
        ...
        <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-8 p-8 pt-10">
          <div>
            <h1 className="flex items-center gap-3 text-3xl font-bold tracking-tight text-white/90">
          ...
          <form onSubmit={uploadDocument} className="rounded-2xl border border-white/[0.08] bg-[#111111] p-6">
            <h2 className="text-lg font-bold text-white/90">Upload Document</h2>
            <div className="mt-5 flex flex-col gap-4 sm:flex-row">
              <Input type="file" accept=".pdf,.docx,.txt" onChange={(event) => setFile(event.target.files?.[0] || null)} className="h-auto border-white/[0.08] bg-white/[0.03]" />
              <Button disabled={!file || !tenantId || status === "uploading"} className="gap-2 bg-[#087f9c] text-white hover:bg-[#0aa1c4]">
                <UploadIcon className="size-4" />
                {status === "uploading" ? "Uploading..." : "Upload"}
              </Button>
            </div>
            {message && <p className={`mt-4 text-sm ${status === "error" ? "text-red-200" : "text-muted-foreground"}`}>{message}</p>}
          </form>

          <section className="rounded-2xl border border-white/[0.08] bg-[#111111] p-6">
            <h2 className="text-lg font-bold text-white/90">Uploaded Documents</h2>
            {uploadedDocs.length === 0 ? (
              <p className="mt-5 text-sm text-muted-foreground">Uploaded documents will appear here for this session.</p>
            ) : (
              <div className="mt-5 space-y-3">
                {uploadedDocs.map((doc) => (
                  <div key={doc.id} className="flex items-center justify-between gap-4 rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
                    <div className="flex min-w-0 items-center gap-3">
                      <FileTextIcon className="size-5 shrink-0 text-[#087f9c]" />
                      <span className="truncate text-sm font-medium text-white/90">{doc.name}</span>
                    </div>
                    <Button onClick={() => deleteDocument(doc.id)} variant="outline" size="sm" className="gap-2 border-white/[0.08] bg-white/[0.03]">
