...
function uptimeStatusClass(status: string) {
  if (status === "up") return "border-emerald-500/25 bg-emerald-500/10 text-emerald-200"
  if (status === "down") return "border-red-500/25 bg-red-500/10 text-red-200"
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                variant="outline"
                size="sm"
                className="gap-2 border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.08]"
              >
            ...
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-5">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-lg font-semibold text-white/90">Connected URL</h2>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            {(["1h", "24h", "7d", "30d"] as const).map((range) => (
              <Button
                ...
                className="h-8 text-xs bg-white/[0.03] border-white/[0.08] hover:bg-white/[0.08]"
              >
                Last {range}
              </Button>
            ))}
          ...
          <div>
              ...
              <div>
                {metricsLoading ? (
                  <div className="space-y-4">
                    <Skeleton className="h-24 rounded-lg bg-white/[0.04]" />
              ...
              <div className="space-y-6">
                {!currentMonitor ? (
                  <div className="flex flex-col items-center justify-center rounded-2xl border border-white/[0.08] bg-white/[0.02] p-12 text-center">
                    ...
                    <h3 className="mt-4 text-xl font-semibold text-white/90">No uptime URL connected</h3>
                  ...
                  <>
                    <div className="grid gap-4 md:grid-cols-3">
                      <div className="rounded-2xl border border-white/[0.08] bg-[#111111] p-5">
                        ...
                        <p className="mt-3 text-2xl font-bold text-white/90">{currentMonitor.currentStatus}</p>
                      </div>
                      <div className="rounded-2xl border border-white/[0.08] bg-[#111111] p-5">
                      ...
                      <div className="rounded-2xl border border-white/[0.08] bg-[#111111] p-5">
                        ...
                        <p className="mt-3 break-all text-sm text-white/90">{currentMonitor.url}</p>
                      </div>
                    </div>
                    ...
                    <div className="rounded-2xl border border-white/[0.08] bg-[#111111] p-5">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <h3 className="text-lg font-semibold text-white/90">Recent checks</h3>
                      ...
                      <div className="mt-5 space-y-3">
                        {uptimeResults.length === 0 ? (
                          ...
                          uptimeResults.slice(0, 10).map((result) => (
                            <div key={result.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                              <div>
                                <p className="font-medium text-white/90">{new Date(result.timestamp).toLocaleString()}</p>
                                <p className="mt-1 text-xs text-muted-foreground break-all">{result.url}</p>
                              </div>
                              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                                <span className={`rounded-lg border px-2.5 py-1 font-semibold uppercase tracking-wide ${result.success ? "border-emerald-500/25 bg-emerald-500/10 text-emerald-200" : "border-red-500/25 bg-red-500/10 text-red-200"}`}>
                                  {result.success ? "Up" : "Down"}
                                </span>
                                <span>{result.statusCode || "--"} status</span>
                                <span>{result.responseMs} ms</span>
                              </div>
                              {result.error && <p className="w-full text-xs text-red-200">{result.error}</p>}
                            </div>
                          ))
                        )}
                      </div>
            ...
            {activeTab === "logs" && (
              <div>
                {logsLoading ? (
                  <div className="space-y-4">
                    <Skeleton className="h-12 rounded-lg bg-white/[0.04]" />
